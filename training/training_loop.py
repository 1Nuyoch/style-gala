...
def training_loop(
    ...
    network_snapshot_ticks  = 25,       # How often to save network snapshots? None = disable.原来是50
    ...
    if (resume_pkl is not None) and (rank == 0):  # 000
    ...
    augment_pipe = None
    ada_stats = None
    if (augment_kwargs is not None) and (augment_p > 0 or ada_target is not None):  # 000
    ...
    for name, module in [('G_encoder', G.encoder), ('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('D', D), (None, G_ema), ('augment_pipe', augment_pipe)]:
        # if module is None:  # **先判断 module 是否为 None**
        #     continue  # 直接跳过，避免 AttributeError

        if (num_gpus > 1) and (module is not None) and len(list(module.parameters())) != 0:     # 000
            module.requires_grad_(True)
            # if hasattr(module, "attn"):
            #     for param in module.attn.parameters():
            #         param.requires_grad = True  # **手动解冻 attn**
            ...
            # if hasattr(module, "attn"):  # 确保 attn 存在
            #     for param in module.attn.parameters():
            #         param.requires_grad = True  # **单独解冻 attn**


            module.requires_grad_(False)

        # for a, b in module.named_parameters():
        #     print(f'{a}====={b.requires_grad}')
    ...
    if rank == 0:
        ...
        pred_images = torch.cat(
            [G_ema(
             ...
             for erased_img, mask, c in zip(grid_erased_img, grid_mask, grid_c)]
        )
    ...
    while True:
        ...
        for phase, phase_gen_c in zip(phases, all_gen_c):  # phases 装有模型 G D参数
        ...
        # Save image snapshot.
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):  # 111
