...
# 定义了数据加载的核心逻辑，用于在工作进程中加载和处理数据样本。
...
# 工作进程管理循环，用于处理从输入队列中接收的任务，并将结果放入输出队列。
...
# 用于将批次数据固定在内存中，以加速在 GPU 上的加载和传输。
...
# 设置 SIGCHLD 信号处理器，用于处理工作进程失败的情况
...
class DataLoaderIter(object):#迭代地加载数据集，由 DataLoader 实例调用
