...
class NormalizeTensor:
    ...
    def __call__(self, tensor):  # tensor：输入的张量图像，大小为 (C, H, W)，其中 C 是通道数，H 和 W 是图像的高度和宽度。
        if not self.inplace:
            tensor = tensor.clone()  # 则克隆输入张量，避免直接修改原始数据。

        dtype = tensor.dtype  # 获取张量的数据类型 dtype
        # 将 mean 和 std 转换为与张量相同的数据类型，并移动到张量所在的设备（GPU 或 CPU）
        mean = torch.as_tensor(self.mean, dtype=dtype, device=tensor.device)
        std = torch.as_tensor(self.std, dtype=dtype, device=tensor.device)
        tensor.sub_(mean[None, :, None, None]).div_(std[None, :, None, None])  # 对张量进行归一化操作：先减去均值 mean，再除以标准差 std。
        return tensor
...
class ModelBuilder:
    ...
    def weights_init(m):
        ...
        if classname.find('Conv') != -1:
            nn.init.kaiming_normal_(m.weight.data)  # 使用 kaiming_normal_ 方法进行权重初始化。
        elif classname.find('BatchNorm') != -1:
            m.weight.data.fill_(1.)  # 则将权重设为 1，偏置设为一个很小的值 1e-4。
            m.bias.data.fill_(1e-4)

    @staticmethod
    def build_encoder(arch='resnet50dilated', fc_dim=512, weights=''):  # 编码器的架构名称，全连接层的维度,预训练模型的权重路径，如果为空则不加载预训练权重。
        pretrained = True if len(weights) == 0 else False
        arch = arch.lower()
        if arch == 'mobilenetv2dilated':  # 则构建 MobileNetV2Dilated 模型。
    ...
    @staticmethod  # 构建指定 arch_decoder 架构的解码器模型，并加载预训练权重。
    ...
    @staticmethod  # 根据 segmentation 是否为 True 来选择是否加载预训练权重。
...
# 构建一个包含 3x3 卷积、Batch Normalization 和 ReLU 激活函数的序列模块。
...
class SegmentationModule(nn.Module):
    def __init__(self,
                 ...
                 drop_last_conv=False,#是否在解码器末端添加 dropout
                 net_enc=None,  # None for Default encoder
                 net_dec=None,  # None for Default decoder
                 encode=None,  # {None, 'binary', 'color', 'sky'}
                 use_default_normalization=False,#是否使用默认的输入归一化。
                 return_feature_maps=False,#是否返回特征图
    ...
    def multi_mask_from_multiclass(self, pred, classes):#将多类别预测结果转换为多个二进制掩码。
        def isin(ar1, ar2):
    ...
    @staticmethod
    def multi_mask_from_multiclass_probs(scores, classes):#将多类别分数转换为多类别概率。
        res = None
        for c in classes:
            if res is None:
                res = scores[:, c]
    ...
    def predict(self, tensor, imgSizes=(-1,),  # (300, 375, 450, 525, 600)
        ...
        with torch.no_grad():
            ...
            for img_size in imgSizes:
                ...
                if self.return_feature_maps:
                    features = features + F.interpolate(fmaps[self.return_feature_maps_level], size=segSize) / len(
                        imgSizes)
...
class ResnetDilated(nn.Module):
    def __init__(self, orig_resnet, dilate_scale=8):#预训练的 ResNet 模型，空洞卷积的倍数，默认为 8
        super().__init__()
